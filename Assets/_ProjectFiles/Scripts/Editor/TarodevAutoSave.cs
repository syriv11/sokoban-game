...
namespace Tarodev
{
    ...
    /// </summary>
    [CustomEditor(typeof(TarodevAutoSaveConfig))]
    public class TarodevAutoSave : Editor
    {
        private static TarodevAutoSaveConfig _config;
        ...
        private static void FetchConfig()
        {
            while (true)
            {
                ...
                if (path == null)
                {
                    AssetDatabase.CreateAsset(CreateInstance<TarodevAutoSaveConfig>(), $"Assets/{nameof(TarodevAutoSaveConfig)}.asset");
                    Debug.Log("A config file has been created at the root of your project.<b> You can move this anywhere you'd like.</b>");
                    continue;
                }

                _config = AssetDatabase.LoadAssetAtPath<TarodevAutoSaveConfig>(path);
        ...
        {
            var paths = AssetDatabase.FindAssets(nameof(TarodevAutoSaveConfig)).Select(AssetDatabase.GUIDToAssetPath).Where(c => c.EndsWith(".asset")).ToList();
        ...
        public static void ShowConfig()
        {
            ...
            var path = GetConfigPath();
            EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath<TarodevAutoSaveConfig>(path).GetInstanceID());
        }
